<template>
  <div>
      <div ref="dropdown" class="relative inline-block" @click="toggleDropdown">
          ...
          <div v-if="isOpen" class="absolute left-0 mt-2 w-60 shadow-lg rounded-lg max-h-80"
              :class="selectTheme.dropdown"
              style="z-index: 999999;">
              <input ref="searchInput" v-model="searchQuery" type="text" placeholder="Search icon..."
                  class="w-full p-2 border-b mb-2 rounded-md focus:outline-none"
                  :class="selectTheme.searchInput"  @click.stop />
              ...
              <div class="grid grid-cols-4 gap-2 p-2 max-h-60 overflow-y-auto" v-if="filteredIcons.length > 0">
                  <button v-for="icon in filteredIcons" :key="icon" @click="selectIcon(icon)"
                      class="p-2 text-center text-xl rounded-md transition"
                      :class="selectTheme.iconList">
                      <i :class="icon"></i>
                  </button>
              </div>
